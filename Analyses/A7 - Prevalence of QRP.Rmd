...
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  ...
  apply(., MARGIN = 1, any_obs) %>% 
  table() %>% prop.test %>% 
  extract(c(4, 6)) %>% unlist
```
...
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  ...
  mutate(qrp_obs = apply(select(., -D2), 1, any_obs)) %>% 
  group_by(D2) %>% 
...
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  select(D2, all_of(QRP_set)) %>%
  mutate(qrp_obs = apply(select(., -D2), 1, any_obs_na)) %>% 
  group_by(D2) %>% 
  summarise(total_count = n(), 
            no_QRP = table(qrp_obs)[1],
            any_QRP = table(qrp_obs)[2], 
